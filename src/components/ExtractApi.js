...
export default function ExtractApi() {
  const [country, setCountry] = useState("EG"); //va définir le pays dans lequel on va voir les webcams
  ...
  const getWebCam = () => {
    ...
    axios
      ...
      .then((data) => {
        console.log(data.data.result.webcams[0]);
        setVideo(data.data.result.webcams[0]);
      });
  };
  //console.log(video);
  //5
